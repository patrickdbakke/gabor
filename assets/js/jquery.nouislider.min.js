...
(function ($) {
	...
	$.fn.noUiSlider = function (options, flag) {
		...
		methods = {
			create : function () {
				...
				return this.each(function () {
					...
					// handles
					handlehtml = '<a class="slidebar"><div></div></a>',
					...
					slider.find(".slidebar").last().addClass("latter");
					if (EVENT == 1) {
			...
			noUiDestroy:function(){
				console.debug("HERE");
				if($(this).data('setup')){
					var handles = $(this).data('setup').handles;
					for (var i = 0; i < handles; i++) {
						handles[i].find('div').unbind();
					}
					$('body').unbind('selectstart.noUiSlider');
					$(this).removeData("_isnS_");
					$(this).removeData("setup");
					$(this).unbind();
					$(this).html('');
				}
				// delete this;
		...
		return options == "disabled" ? methods.disabled.apply(this) : (options=="delete" ? methods.noUiDestroy.apply(this) : methods.create.apply(this));
